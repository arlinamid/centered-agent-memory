...
- ~~**`recall_events`**~~ — már nem alszik: az M3 memória-rétege ebből promótál
  ([`memory.md`](memory.hu.md)). Jól tette, hogy az első naptól gyűlt.
...
`cam memory` parancsok és `cam_memory` MCP-tool. Hogyan működik: [`memory.md`](memory.hu.md).
...
[`operations.md`](operations.hu.md). Tételes lista a [`CHANGELOG.md`](../CHANGELOG.md) „M4 —
Üzemeltetés" szakaszában.

A mérföldkő feltételei, mérve:

- **A sync felügyelet nélkül fut, és hibát nem nulla kilépési kóddal jelez.** Ütemezési minta mind a
  négy platformra ([`operations.md`](operations.hu.md)), és `--quiet`, ami hiba esetén beszél, egyébként
...
[`install.md`](install.hu.md). Tételes lista a [`CHANGELOG.md`](../CHANGELOG.md) „Telepítés" szakaszában.
...
bizonyíték-táblát sem ír. Hogyan: [`memory.md`](memory.hu.md#az-álom-fázis-opcionális).
...
megvannak. Részletek: [`operations.md`](operations.hu.md).
...
6. Dokumentáld a formátumot és a buktatóit a [`sources.md`](sources.hu.md)-ben.
